...
function layout_url($rpath = null)
{
	...
	if(is_dir( APPPATH.'views/'.$layoutpath)){
		
		return (!is_null($rpath))? base_url() . 'application2/views/' . $layoutpath :base_url() . 'application2/views/';
	}else{
		
		return (!is_null($rpath))? base_url() . 'application2/views/' . $rpath :base_url() . 'application2/views/';
	}
}
...
function store_fallback_path($rpath = null)
{
    ...
    if($store[0]['is_custom_theme']){
		...
		if(file_exists( APPPATH.'views/'.$layoutpath)){
			...
			return (!is_null($rpath))? base_url() . 'application2/views/default/' . $rpath :base_url() . 'application2/views/';
		}
    }
	return (!is_null($rpath))? base_url() . 'application2/views/default/' . $rpath :base_url() . 'application2/views/';
}
