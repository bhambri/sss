...
class Cart extends VCI_Controller {
				...
				if($payusing){
				 ?>
					...
					<form action="https://webservice.paymentxp.com/wh/EnterPayment.aspx" method="POST" id="payxp">
						...
						<input type="text" name="TransactionType" value="CreditCardCharge" />
						...
						<input type="text" name="PostBackURL" value="<?php echo site_url().'cart/success?method=meritus' ?>" />
	...
	{   
        ...
        #$store = $this->common_model->get_clientdetail($storeuser);
		$storeid = $store[0]['id'] ;
		$storeusername = $store[0]['username'] ;
		$is_custom_theme = $store[0]['is_custom_theme'] ;

		if($is_custom_theme){
            $this->_vci_layout('clientstore_default' ,$storeusername.'_'.$storeid);
        }else{
            $this->_vci_layout('clientstore_default');
        }
		...
		if ( $this->input->post( 'formSubmitted' ) > 0 )
        {
            ...
            if ( $this->form_validation->run('checkout_form') )
            {
				...
				*/
				...
				//11052015 -- nw code to maritus payment gateway payusing - 1 meritus , 0 -paypal
				//if(strtolower($this->input->post('payusing') ) == 'meritus'){ 
				if($payusing){
					?>
					<div style="display:none;">
					...
					<form action="https://webservice.paymentxp.com/wh/EnterPayment.aspx" method="POST" id="payxp">
						...
						<input type="text" name="MerchantKey" value="<?php echo $store_settings[0]->mp_merchant_key ;?>" />
						<input type="text" name="TransactionAmount" value="<?php echo $GrandTotal ; ?>" /></td>
						<input type="text" name="PostBackURL" value="<?php echo site_url().$consultantDetail.'/cart/success?method=meritus' ?>" />
						<input type="submit" value="Submit" />
					</form>
					<script>
						jQuery("#payxp").trigger('submit');
	...
	{	
		...
		if( !empty( $storeUserSession ) && is_array( $storeUserSession ) )
		{
			...
			$view_data['shipping_cost'] = $ShippinCost = 0 ; // Abhishek commented on 15-04-2015
		}
	...
	function consultantmanage(){
		...
		if( !empty( $storeUserSession ) && is_array( $storeUserSession ) )
		{
			...
			else
			{
			 ...
			 //$view_data['shipping_cost'] = $ShippinCost = $this->getShippingAmount( $state_code ); // Abhishek commented on 15-04-2015
	...
	function success()
	{
		...
		if(@$_GET['method'] == 'meritus'){
			...
			if( $this->common_model->insert('order', $data ) ){
    			...
    			redirect( site_url().'cart/thanks?tid=' . $transactionid );
    			exit;
			}
			
		}
		////// new section to edited ends here
	...
	function consultantsuccess()
	{
		...
		// new section for meritus pay ends here
		...
		if(isset($_GET["token"]) && isset($_GET["PayerID"]))
		{
			...
			$PayPalApiSignature 	= $store_settings[0]->paypal_signature;  //Paypal API Signature
			$PayPalCurrencyCode 	= 'USD'; //Paypal Currency Code
			$PayPalReturnURL 		= site_url().$consulttantDetail.'/cart/success'; 
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
			{
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
				{
					echo '<div> Please contact us if you faced difficulty with payment no.'.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]).'</div>' ;
				}else {
					echo '<div style="color:red"><b>GetTransactionDetails failed:</b>'.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]).'</div>';
				}
	...
	/* function send email on order completion with detail 
	*/
		...
		<table border='1' cellpadding='10px'>
			...
			foreach ($order_detail as $key => $value) 
			{
				$order_item .= "<tr>
		       	...
		       	</tr><tr><td>Specifications: </td><td colspan='3'>".$value->product_specification."</td></tr>";
		     }
		$order_item .= "</table>
		";
	...
	function refundpay(){
        ...
        (
            ...
            "ReferenceNumber" => "",
            "TransactionAmount" => "70.99",
            "TransactionID" => "23298150",
