...
class Address extends VCI_Controller {
	...
	function shipping()
	{
		...
		$owner_details = $this->client_model->get_client_detailsurl( $storeurl );

        $storeUsername =  $owner_details->username;
            ...
            if( $data['formsubmitted'] > 0 && !empty( $data['formsubmitted'] ) )
            {
                if ( $this->form_validation->run('shipping_address') )
                {
                   ...
                   $data_array = array( 
                    ...
                    if( $this->address_model->store_shipping_information( $data_array, $user_id ) )
                    {
                       redirect( site_url() .$this->uri->segment(1).'/user/account' );
                    }
                }         
            }
        }
    ...
    function cbilling()
    {
        ...
        if($is_custom_theme){
            $this->_vci_layout('clientstore_default' ,$storeUsername.'_'.$store_id);
        }else{
            $this->_vci_layout('clientstore_default');
        }
