...
class Messages extends VCI_Controller {
	...
	function compose() {
		if($this->input->post('subject')) {
			if($this->input->post('participant') && !empty($this->input->post('participant'))) {
				if($this->input->post('message') && !empty($this->input->post('message'))) {
					...
					$this->session->set_flashdata('errors', 'Please enter the message.');
					redirect('/messages/compose', 'refresh');
				}
			} else {
				$this->session->set_flashdata('errors', 'Kindly specify the receiptant.');
				redirect('/messages/compose', 'refresh');
			}
		}
