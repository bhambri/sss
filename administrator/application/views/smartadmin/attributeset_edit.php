...
<div class="jarviswidget jarviswidget-sortable" id="wid-id-2" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false" role="widget">
	...
	<div role="content">
		...
		<div class="widget-body no-padding">
			<?php echo form_open_multipart('attributesets/edit/id/'.$this->cat_id ,array('id'=>'formEditclient','name'=>'formEditclient', 'class'=>"smart-form"));?>
				<fieldset>
					<section>
						<label class="label">Name:<span class="mandatory">*</span></label>
						<label class="input">
							<input type="text" name="name" id="name"  maxlength = "50" class="input-xs" value="<?php echo (isset($attributeset->name) ? $attributeset->name : set_value('fName',''));?>" />
						</label>
					</section>
					<section>
						<label class="label">Description:</label>
						<label class="textarea">
							<textarea name="description" id="description" cols="30" rows="4"><?php echo (isset($attributeset->description) ? $attributeset->description : set_value('description',''));?></textarea>
						</label>
					</section>
					<section>
						<label class="checkbox">
							<input type="checkbox" name="status" id="status" class="inputbox" value="1" <?php echo ((isset($attributeset->status) ? $attributeset->status : set_value('status','')) == 1)?'checked="checked"':''; ?> />
							<i></i>Active
						</label>
					</section>
				</fieldset>
