...
class Mahana_model extends CI_Model
{
    ...
    {
        $sql = 'SELECT m.*, DATE_FORMAT((m.cdate), "%d %b %Y %l:%i %p") as cdate, s.status, t.subject, '.USER_TABLE_USERNAME .
     ...
     * @return  array
     */
    function get_all_threads($user_id, $full_thread = FALSE, $order_by = 'asc', $last_record_number  = 0)
    {
		$sql	= 'SELECT 
					m.id, min(m.thread_id) as thread_id, t.subject, m.body, m.priority, m.sender_id, DATE_FORMAT(max(m.cdate), "%d %b %Y %l:%i %p") as cdate, min(s.status) as status, '. USER_TABLE_USERNAME .' 
					FROM `msg_messages` as m 
					JOIN msg_threads t ON (t.id = m.thread_id)  
					join msg_participants as p on m.thread_id = p.thread_id 
					join msg_status s ON (s.message_id = m.id and s.user_id = ? ) 
					JOIN ' . USER_TABLE_TABLENAME . ' ON ' . USER_TABLE_ID . ' = m.sender_id 
					where p.user_id = ? 
					group by m.thread_id ';
					
		$sql 	.= ' ORDER BY max(m.cdate) ' . $order_by. ' ';
		$sql	.= ' limit '.$last_record_number.',10 ';

        $query = $this->db->query($sql, array($user_id, $user_id));
        return $query->result_array();
    }
	
	function get_all_threads_count($user_id)
    {
		$sql	= 'SELECT COUNT(DISTINCT thread_id) as count FROM `msg_participants` where `user_id` = ? ';
        $query = $this->db->query($sql, array($user_id));
		$result	= $query->result_array();
		
		if(count($result) && isset($result[0]['count'])) {
			return $result;
		} else {
			return [['count' => 0]];
		}
	...
	function mark_thread_read($thread_id, $user_id) {
		$sql	= 'update `msg_status` 
					set status = ' . MSG_STATUS_READ . ' 
					where `msg_status`.`message_id`  IN (SELECT id FROM msg_messages where thread_id  = ? ) 
					and user_id = ?  ';
