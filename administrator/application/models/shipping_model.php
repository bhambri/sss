...
/*
---------------------------------------------------------------
*	Class:	shipping_model extends Model defined in Core libraries
*	Author: 
...
class Shipping_model extends CI_Model {
	...
	function add( $store_id ){
        if($this->input->post('formSubmitted') > 0){
        	...
        	$data = array(
        			...
        			'store_id'    => $store_id,
        			'w1'          => htmlspecialchars($this->input->post('w1', true)),
        			'w2'          => htmlspecialchars($this->input->post('w2', true)),
        			'w3'          => htmlspecialchars($this->input->post('w3', true)),
        			'w4'          => htmlspecialchars($this->input->post('w4', true)),
        			'w5'          => htmlspecialchars($this->input->post('w5', true)),
        			'created'     => date("Y-m-d h:i:s")
        	);
			
        	$this->db->insert('shipping_costs', $data);
	...
	**/
	function update($id = null, $store_id) {
		...
		$this->db->where('id', $id);
		$this->db->update('shipping_costs', $data);
		return true;
	...
	*********************************************************************
	**/
	function get_shipping_page($id = null, $store_id)
	{
		if(is_null($id)) {
			return false;			
		} else {
			$result = $this->db->get_where('shipping_costs',array('id'=>$id,'store_id'=>$store_id));
		}
	...
	*/
	...
	function get_shipping_state($page, $per_page = 10, $count = false,$qstr='',$store_id) {
		...
		if(count($content->result()) > 0)
		{
			...
			foreach ($content->result() as $key => $value) {
				...
				$this->db->where('shipping_costs.state_code', $value->state_code );
				$this->db->where('shipping_costs.store_id' , $store_id);
				$cn = $this->db->get();
	...
	**/
	function update_status($id = null, $status = 0)
	{
		...
		$this->db->update('shipping', array('status'=>$status));
		return true;
	}
	...
	**/
	function remove_image($id){
		$this->db->where(array('id'=>$id));
		$this->db->update('shipping', array('page_thumbnailpath'=>''));
		return true;
	}
	...
	**/
	function count_shipping(){
		$total = $this->db->count_all('shipping');
		return $total;
	}
