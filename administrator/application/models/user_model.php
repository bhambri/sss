...
class User_model extends VCI_Model {
	...
	**/
	...
	function get_all_duereport($page, $per_page = 10,$count=false, $store_id = false, $role_id = null, $date_from="", $date_to="" )
	{
		...
		$this->db->select(array('allcommisions.*','users.name','users.username', 'users.meritus_customer_id', 'settings.mp_merchant_id', 'settings.mp_merchant_key', 'settings.meritus_enabled'));
		$this->db->order_by("id", "desc");
		//$this->db->limit($per_page, $page);
	...
	*/
	...
	function update_status_allcommisions($id, $payment_mode = 'offline') {
		...
		$participant	= ['U'.$data['consultant_id']];
		
		$amount	= $data['all_due'];
		switch($payment_mode) {
			case 'meritus':
				$this->load->library('meritus');
				$response	= $this->meritus->makeCredit($data['meritus_customer_id'], $data['mp_merchant_id'], $data['mp_merchant_key'], $amount);
				$payment_mode	= 'Meritus';
				break;
			default:
		...
		$this->load->helper('vci_common');
		$subject	= 'Dues Transfered';
		$message	= 'Transfer Details <br />Payment Method : ' . $payment_mode . '<br />Amount : ' . $amount . '<br />';
		...
		if($response['TransactionID']) {
			...
			if($this->db->insert('transactions', $transaction)) {
				...
				$message	.= 'Status : Successfully Done <br />';
				$this->mahana_messaging->send_new_message($user['user_id'], $participant, $subject, $message, 10);
				return true;
			}
		}
		$message	.= 'Status : Error <br />';
		$this->mahana_messaging->send_new_message($user['user_id'], $participant, $subject, $message, 10);
		return false;
	}
