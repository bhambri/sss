...
class Mahana_messaging
{
	...
	function delete($thread_id, $user_id) {
		
        if ($message = $this->ci->mahana_model->delete($thread_id, $user_id))
        {
            return $this->_success($message);
        }
    ...
    function get_all_threads($user_id, $full_thread = FALSE, $order_by = 'ASC', $last_record_number = 0)
    {
		...
		$message = $this->ci->mahana_model->get_all_threads($user_id, $full_thread, $order_by, $last_record_number);
        if (is_array($message))
        {
            return $this->_success($message);
        }
	 ...
	 function get_all_threads_count($user_id)
    {
		...
		$message = $this->ci->mahana_model->get_all_threads_count($user_id);
        if (is_array($message))
        {
            return $this->_success($message);
        }
	...
	function mark_thread_read($thread_id, $user_id) {
		...
		$message = $this->ci->mahana_model->mark_thread_read($thread_id, $user_id);
        if (is_array($message))
        {
            return $this->_success($message);
        }
