...
error_reporting(0);
	...
	$banners = $this->common_model->get_banner($storeid,$store_role) ;
        ...
        $news_data = $this->news_model->get_consultantnews($storeid,$consultantDetail[0]->id,0,5,false) ;
       $popularcat = $this->common_model->getpopularsubcatlist($storeid);
		//$this->load->model('common_model');
		$view_data['title'] = 'Home ' ;
		$view_data['banners'] = $banners;
	...
	*/
	...
	function clientproducts($page = 0 ){
		...
		$view_data['breadcrumbdata'] = array('Home /'=> base_url() .$store_username ."/".$consultantuser."/home" ,'Store'=>'') ;
	...
	{	
		...
		//Fetch all pages from database
		$view_data['breadcrumbdata'] = array('Home /'=> base_url() .$store_username ."/home" ,'Store'=>'') ;
	...
	function catproducts(){
		...
		if((@$categoryDetail[0]->name !='') && ($subcategoryDetail[0]->name !='' )){
			$view_data['breadcrumbdata'] = array(
							'Home /'=> base_url() .$store_username ."/home",
    ...
    function clientcatproducts(){
		...
		if((@$categoryDetail[0]->name !='') && ($subcategoryDetail[0]->name !='' )){
			$view_data['breadcrumbdata'] = array(
							'Home /'=> base_url() .$store_username."/".$segs[2]."/home",
