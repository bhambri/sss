...
class Contactus extends VCI_Controller {
    ...
    {	
        ...
        if($is_custom_theme){
            $this->_vci_layout('store_default' ,$store_username.'_'.$storeid);
        }else{
        ...
        if( @$this->store_id > 0 ){
            ...
            if(!empty($storedata)){
                ...
                $view_data['phone1'] = $storedata[0]['phone'];
...
//echo '<pre>';
    ...
    function cclient()
    {
        ...
        $popularcat = $this->common_model->getpopularsubcatlist($this->store_id) ;
        $consultantDetail = $this->user_model->is_consultant_exists(trim($this->uri->segments[2]), $this->store_id) ;
        ...
        // store categories ends here
        // rule name conatctus
        $view_data['breadcrumbdata'] = array('Home /'=> base_url() .$storeuser .'/'.$this->uri->segment(2)."/home" ,
                'Contact'=>'') ;
