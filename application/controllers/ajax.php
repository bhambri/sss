...
class Ajax extends VCI_Controller {
	...
	function addtocart($productid){
		...
		if(!empty($_POST)){
			...
			$productatrr = $this->product_model->getassignedattribute($productId) ;
			if(!empty($productatrr)){
				echo 2; die; // move to product detail page
			}
	...
	function updateordercomment(){
		$this->load->library('cart');
		if(isset( $_POST['cart_comments'] ) && !empty( $_POST['cart_comments'] ) ){
        $this->cart->update_order_comment($_POST['cart_comments']);
        }
        return true;
	}
