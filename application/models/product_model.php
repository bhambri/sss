...
class Product_model extends VCI_Model{
	...
	**/
	...
	function get_all($page, $per_page, $count = false,$sort_by='id',$category_id='',$subcategory_id='') {
		...
		$wherestoreid = "client_product.store_id = $this->store_id" ;
		$this->db->where($wherestoreid);

		$wherestat = "client_product.status = 1" ;
		$this->db->where($wherestat);

		if($category_id){
			
			$wherecat = "client_product.category_id = $category_id" ;
			$this->db->where($wherecat);
		}
		if($subcategory_id){
			
			$wherescat = "client_product.subcategory_id = $subcategory_id" ;
			$this->db->where($wherescat);
		}
		
		if(strlen($qstr) > 0) {
			
			$wheren = "(client_product.sku LIKE '%$qstr%' OR client_product.product_title LIKE '%$qstr%')";
	...
	function get_allold($page, $per_page, $count = false,$sort_by='id',$category_id='',$subcategory_id='') {
	...
	*/
	...
	function getassignedattribute($productid){
		$this->db->from('client_product_attribute_sets') ;
		$this->db->join('attribute_sets' , 'attribute_sets.id = client_product_attribute_sets.attribute_set_id' ) ;
		$this->db->where('client_product_attribute_sets.client_product_id' , $productid);
		$this->db->where('attribute_sets.status',1);
		$prodlist = $this->db->get();
		#echo '<pre>';
		#print_r($prodlist);
		$attrSet =  array() ;
		 if(!empty($prodlist->num_rows)){
			foreach ($prodlist->result() as $key => $value) {
				
				$this->db->from('attribute_set_fields');
				
				//$this->db->join('attribute_set_field_details' , 'attribute_set_field_details.attribute_set_field_id = attribute_set_fields.id' ) ;
				$this->db->where('attribute_set_id' , $value->attribute_set_id );
				$optlist = $this->db->get();
				$allopt = array();
				if($optlist->num_rows){
					$rsset = $optlist->result() ;
					#echo '<pre>';
					#print_r($rsset);
					
					foreach($rsset as $newrset){
						#echo '<pre>';
						#print_r($newrset);
						if($newrset->field_type == 'text'){
							$optdetails =  array('option_detail'=>array()) ;
						}else{
							$this->db->from('attribute_set_field_details');
							$this->db->where('attribute_set_field_id' , $newrset->id );
							$rssetopt = $this->db->get();
							$rssetopt = $rssetopt->result_array() ;
							#echo '<pre>';
							#print_r($rssetopt) ;
							$optdetails =  array('option_detail'=>$rssetopt ) ;
						}
						
						# $optdetails = array() ;
						$allopt[] = array_merge((array) $newrset ,$optdetails );
					}
					#echo '<pre>';
					#print_r($allopt);
				}
				$attrSet[$value->attribute_set_id] = $allopt ;
	...
	function getattributefielddetail($id){
		// attribute_set_fields
		$this->db->from('attribute_set_field_details');
		$this->db->where('id' , $id);
		$optlist = $this->db->get();
	...
	function getProductVolume($id){
		// attribute_set_fields
		$this->db->select('product_volume');
		$this->db->from('client_product');
		$this->db->where('id' , $id);
		$optlist = $this->db->get();
		$allopt = array();
