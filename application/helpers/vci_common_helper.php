...
// Replacement of the function -- $co->load->view() 
function svci_load_view($path='null'){
	$co = & get_instance() ;
    ...
    if($store[0]['is_custom_theme'] == 1){
    	$layoutpath  = trim(strtolower($co->storename)).'_'.trim(strtolower($co->store_id)).'/' . trim(strtolower($path)).'.php' ;
		if(file_exists( APPPATH.'views/'.$layoutpath)){
			$co->load->view(trim(strtolower($co->storename)).'_'.trim(strtolower($co->store_id)).'/'.$path);
		}else{
			$co->load->view('default/'.$path);
		}
